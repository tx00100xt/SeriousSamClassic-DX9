...
void CTextureData::SetAsCurrent( INDEX iFrameNo/*=0*/, BOOL bForceUpload/*=FALSE*/)
{
  ...
  if((td_ctFrames>1 && td_pulObjects==NULL) || (td_ctFrames<=1 && td_ulObject==NONE))
  {
    ...
    if( td_ctFrames>1) {
      // animation textures
      td_pulObjects = (ULONG64*)AllocMemory( td_ctFrames *sizeof(td_ulProbeObject));
  ...
  ULONG64 ulTexObject = (td_ctFrames>1) ? td_pulObjects[iFrameNo] : td_ulObject; // single-frame or animation
  if( bUseProbe) {
...
SLONG CTextureData::GetUsedMemory(void)
{
  ...
  ULONG64 ulTexObject = (td_ctFrames>1) ? td_pulObjects[0] : td_ulObject;
