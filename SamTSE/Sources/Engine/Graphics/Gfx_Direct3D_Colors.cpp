...
pixLoop:
	...
	const ULONG slRowModulo = slPitch - (pixWidth << 2);
	ULONG *esi = pulSrc;
	ULONG *edi = static_cast<ULONG*>(pulDst);
	ULONG ebx = pixHeight;
	while (ebx != 0) {
		ULONG ecx = pixWidth;
		while (ecx != 0) {
...
#else
	// ### c++ implementation ###
	const int32_t slRowModulo = slPitch - (pixWidth << 1);
	uint32_t ulAlphaMask = 0xFF000000;
	uint16_t *dst = static_cast<uint16_t*>(pulDst);

	for (int32_t row = 0; row < pixHeight; row++)
	{
		for (int32_t col = 0; col < pixWidth; col++)
		{
			uint32_t pixel = pulSrc[row * pixWidth + col];
			pixel |= ulAlphaMask;
			uint32_t r = (pixel & 0x000000F8) << 7;
			uint32_t g = (pixel & 0x0000F800) >> 6;
			uint32_t b = (pixel & 0x00F80000) >> (16 + 3);
			uint32_t a = (pixel & 0x80000000) >> 16;
			uint16_t result = static_cast<uint16_t>(r | g | b | a);
			*dst = result;
			dst++;
		}
		dst += slRowModulo / 2;
...
pixLoop:
	...
	for (int32_t row = 0; row < pixHeight; row++)
	{
		for (int32_t col = 0; col < pixWidth; col++)
		{
			uint32_t pixel = pulSrc[row * pixWidth + col];
			pixel |= ulAlphaMask;
			uint32_t r = (pixel & 0x000000F0) << 4;
			uint32_t g = (pixel & 0x0000F000) >> 8;
			uint32_t b = (pixel & 0x00F00000) >> (16 + 4);
			uint32_t a = (pixel & 0xF0000000) >> 16;
			uint16_t result = static_cast<uint16_t>(r | g | b | a);
			*dst = result;
			dst++;
		}
...
pixLoop:
	...
	// ### c++ implementation ###
	const int32_t slRowModulo = slPitch - (pixWidth << 1);
	UINT_PTR pulDst_ptr = (UINT_PTR)pulDst;
	for (int32_t row = 0; row < pixHeight; row++) {
		for (int32_t col = 0; col < pixWidth; col++) {
			uint32_t pixel = pulSrc[row * pixWidth + col];
			uint32_t r = (pixel & 0x000000F8) << 8;
			uint32_t g = (pixel & 0x0000FC00) >> 5;
			uint32_t b = (pixel & 0x00F80000) >> (8 + 5 + 6);
			uint16_t rgb565 = r | g | b;
			*(uint16_t *)(/* pulDst */ pulDst_ptr + row * slPitch + col * 2) = rgb565;
		}
	}
#endif
}
...
pixLoop:
	...
	// ### c++ implementation ###
	const int32_t slRowModulo = slPitch - (pixWidth << 1);
	uint32_t *esi = (uint32_t *)pulSrc;
	uint16_t *edi = (uint16_t *)pulDst;
	int32_t ebx = pixHeight;

	while (ebx > 0) {
		int32_t ecx = pixWidth;
		while (ecx > 0) {
			uint32_t eax = *esi;
			eax = (eax >> 8) | (eax << 24);
			//eax = __builtin_bswap32(eax);
...
pixLoop:
	...
	const ULONG slRowModulo = slPitch - (pixWidth << 0);
	ULONG *esi = pulSrc;
	uint8_t *edi = static_cast<uint8_t*>(pulDst);
	ULONG ebx = pixHeight;
	while (ebx != 0)
	{
		ULONG ecx = pixWidth;
